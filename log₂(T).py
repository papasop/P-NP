...
# 显示 λₖ(x) 和 log₂(n) 对应的分子和分母
df['log2T_pred'] = model.predict(X)
df['lambda_k_contribution'] = coefficients[0] * df['lambda_k']
df['log2n_contribution'] = coefficients[1] * df['log2n']

# 打印 λₖ(x) 和 log₂(n) 对 log₂(T) 的贡献
print("\n=== Contributions to log2T ===")
print(df[['lambda_k', 'log2n', 'lambda_k_contribution', 'log2n_contribution']])
